...
auth.normalizeDriveProfileImageUrl = function (u) {
  ...
  if (u.indexOf('/image/cache/') >= 0) return u;

  function toLocalCache(id) {
    if (!id) return u;
    var base = './image/cache/';
    if (typeof window !== 'undefined') {
      var p = window.location.pathname || '';
      if (p.indexOf('/imboxer/') >= 0) base = '../image/cache/';
      else if (p.indexOf('/frontend/') >= 0) base = '/frontend/image/cache/';
    }
    return base + id + '.png';
  }

  var lh3 = u.match(/lh3\.googleusercontent\.com\/d\/([a-zA-Z0-9_-]+)/);
  if (lh3) return toLocalCache(lh3[1]);
  if (u.indexOf('drive.google.com/uc?export=view&id=') >= 0) {
    var um = u.match(/id=([a-zA-Z0-9_-]+)/);
    if (um) return toLocalCache(um[1]);
  }
