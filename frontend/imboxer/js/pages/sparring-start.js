...
function ensureAiVideoLoaded() {
  if (!ui.aiVideo) {
    return;
  }

  const source = ui.aiVideo.dataset.videoSrc;
  if (!source || ui.aiVideo.getAttribute("src") === source) {
    return;
  }
...
async function loadActiveSparringVideo() {
  try {
    const response = await authFetch(`/api/sparring/active-video?mode=${encodeURIComponent(state.mode)}`);
...
function hasCommittedPunchMotion(landmarks, now, actionKey) {
  ...
  const leftDelta = distance2D(leftWrist, previous.leftWrist);
  const rightDelta = distance2D(rightWrist, previous.rightWrist);
  const leftSpeed = (leftDelta / shoulderSpan) / dt;
  const rightSpeed = (rightDelta / shoulderSpan) / dt;
  const leftOutward = leftWrist && previous.leftWrist
    ? previous.leftWrist.x - leftWrist.x
    : 0;
  const rightOutward = rightWrist && previous.rightWrist
    ? rightWrist.x - previous.rightWrist.x
    : 0;
  const leftReachGain = leftWrist && previous.leftWrist && leftShoulder && previous.leftShoulder
    ? distance2D(leftWrist, leftShoulder) - distance2D(previous.leftWrist, previous.leftShoulder)
    : 0;
  const rightReachGain = rightWrist && previous.rightWrist && rightShoulder && previous.rightShoulder
    ? distance2D(rightWrist, rightShoulder) - distance2D(previous.rightWrist, previous.rightShoulder)
    : 0;
  const punchMotionSpeed = toNumber(POSE_TUNING.punchMotionSpeed, 1.75);
  const hookMotionSpeed = toNumber(POSE_TUNING.hookMotionSpeed, punchMotionSpeed + 0.25);
  const minOutward = shoulderSpan * toNumber(POSE_TUNING.punchOutwardGain, 0.035);
  const minReachGain = shoulderSpan * toNumber(POSE_TUNING.punchReachGain, 0.045);
  const minAnyMotion = shoulderSpan * 0.032;
  const committedLeft =
    leftSpeed >= punchMotionSpeed &&
    (leftOutward > minOutward || leftReachGain > minReachGain || leftDelta > minAnyMotion);
  const committedRight =
    rightSpeed >= punchMotionSpeed &&
    (rightOutward > minOutward || rightReachGain > minReachGain || rightDelta > minAnyMotion);

  if (actionKey === "jab") {
    return committedLeft;
  }
  if (actionKey === "cross") {
    return committedRight;
  }
  if (actionKey === "hook") {
    return Math.max(leftSpeed, rightSpeed) >= hookMotionSpeed &&
      (Math.max(Math.abs(leftOutward), Math.abs(rightOutward)) > minOutward * 1.2 ||
      Math.max(leftReachGain, rightReachGain) > minReachGain ||
      Math.max(leftDelta, rightDelta) > minAnyMotion);
  }
  return false;
}
...
function emitPoseAction(snapshot, landmarks, now) {
  ...
  const detectedAction = normalizePoseAction(snapshot?.detectedAction || snapshot?.detectedActionLabel);
...
function bindTimelineVideoMarkers() {
  ...
  ui.aiVideo.addEventListener("timeupdate", () => {
    if (state.gameState !== "round_active" || !state.timelineMarkers.length) return;
    const currentTime = toNumber(ui.aiVideo.currentTime, 0);
    if (currentTime + 0.5 < state.timelineLastTime) {
      state.firedTimelineMarkerIds = new Set();
    }
    state.timelineLastTime = currentTime;
    state.timelineMarkers.forEach((marker) => {
      if (state.firedTimelineMarkerIds.has(marker.id)) return;
      const triggerTime = Math.max(0, marker.impact_time + toNumber(marker.impact_delay_ms, 0) / 1000);
...
async function init() {
  ...
  if (ui.aiVideo) {
    ui.aiVideo.removeAttribute("src");
  }
}
