...
function walk(dir, acc) {
  ...
  for (const name of fs.readdirSync(dir)) {
    ...
    else if (name.endsWith(".html") || name.endsWith(".css")) acc.push(p);
  }
  return acc;
}
...
(async () => {
  ...
  for (const p of htmls) {
    ...
    const after = before.replace(
      ...
      function (_m, id, offset, str) {
        var prefix = "./image/cache/";
        if (offset > 0 && str[offset - 1] === "(") {
          var before = str.slice(Math.max(0, offset - 24), offset);
          if (before.includes("../")) prefix = "../image/cache/";
        }
